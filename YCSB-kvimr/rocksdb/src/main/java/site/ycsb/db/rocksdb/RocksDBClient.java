...
public class RocksDBClient extends DB {
  ...
  @GuardedBy("RocksDBClient.class") private static KVIMREnv sharedKvimrEnv = null;
  @GuardedBy("RocksDBClient.class") private static int references = 0;

  private KVIMREnv kvimrEnv = null;
  ...
  public void init() throws DBException {
    synchronized(RocksDBClient.class) {
      if(rocksDb == null) {
        ...
        } catch (final IOException | RocksDBException | DBException e) {
          throw new DBException(e);
        }
      } else {
        kvimrEnv = sharedKvimrEnv;
      }
   ...
   */
  private RocksDB initRocksDBWithOptionsFile()
      throws IOException, RocksDBException, DBException {
    ...
    configureKVIMR(options);
    disableKVIMRCompression(cfDescriptors);
    dbOptions = options;
   ...
   * @return The initialized and open RocksDB instance.
   */
  private RocksDB initRocksDB() throws IOException, RocksDBException, DBException {
    if(!Files.exists(rocksDbDir)) {
      Files.createDirectories(rocksDbDir);
    }
    ...
    for(final String cfName : cfNames) {
      ...
      disableKVIMRCompression(cfOptions);
    ...
    if(cfDescriptors.isEmpty()) {
      ...
      configureKVIMR(options);
      disableKVIMRCompression(options);
      dbOptions = options;
  ...
  private void configureKVIMR(final Options options)
      throws RocksDBException, DBException {
    if (!Boolean.parseBoolean(getProperties().getProperty("kvimr.enabled", "false"))) {
      return;
    }
    if (sharedKvimrEnv == null) {
      sharedKvimrEnv = createKVIMREnv();
    }
    kvimrEnv = sharedKvimrEnv;
    options.setEnv(kvimrEnv);
  }

  private boolean isKVIMREnabled() {
    return Boolean.parseBoolean(getProperties().getProperty(
        "kvimr.enabled", "false"));
  }

  private void disableKVIMRCompression(
      final List<ColumnFamilyDescriptor> cfDescriptors) {
    if (!isKVIMREnabled()) {
      return;
    }
    for (final ColumnFamilyDescriptor descriptor : cfDescriptors) {
      disableKVIMRCompression(descriptor.getOptions());
    }
  }

  private void disableKVIMRCompression(final Options options) {
    if (!isKVIMREnabled()) {
      return;
    }
    options.setCompressionType(CompressionType.NO_COMPRESSION);
    options.setBottommostCompressionType(CompressionType.NO_COMPRESSION);
    options.setCompressionPerLevel(noCompressionPerLevel(options.numLevels()));
  }

  private void disableKVIMRCompression(final ColumnFamilyOptions options) {
    if (!isKVIMREnabled()) {
      return;
    }
    options.setCompressionType(CompressionType.NO_COMPRESSION);
    ...
    options.setCompressionPerLevel(
        noCompressionPerLevel(options.numLevels()));
  }

  private List<CompressionType> noCompressionPerLevel(final int numLevels) {
    final int levelCount = numLevels > 0 ? numLevels : 7;
  ...
  private void configureKVIMR(final DBOptions options)
      throws RocksDBException, DBException {
    if (!Boolean.parseBoolean(getProperties().getProperty("kvimr.enabled", "false"))) {
      return;
    }
  ...
  private KVIMREnv createKVIMREnv() throws RocksDBException, DBException {
    final String device = getProperties().getProperty(
        "kvimr.device", "/dev/mapper/imrsim");
    final String rmwMode = getProperties().getProperty(
        "kvimr.rmw_mode", "naive");
    final String zonesString = getProperties().getProperty(
        "kvimr.zones", "159");
    final long zones;
    try {
      zones = Long.parseLong(zonesString);
    } catch (final NumberFormatException e) {
      throw new DBException("Invalid kvimr.zones value: " + zonesString, e);
    }
    return new KVIMREnv(rocksDbDir.toAbsolutePath().toString(), device,
        zones, rmwMode);
  }
  ...
  private void flushKVIMR() throws DBException {
    try (final FlushOptions flushOptions =
             new FlushOptions().setWaitForFlush(true)) {
      for (final Map.Entry<String, ColumnFamily> entry : COLUMN_FAMILIES.entrySet()) {
        rocksDb.flush(flushOptions, entry.getValue().getHandle());
      }

      if (!COLUMN_FAMILIES.containsKey("default")) {
        try (final ColumnFamilyHandle defaultHandle = rocksDb.getDefaultColumnFamily()) {
          rocksDb.flush(flushOptions, defaultHandle);
        }
      }
  ...
  private void printKVIMRStats() {
    if (sharedKvimrEnv == null) {
      return;
    }
    System.out.println("[KVIMR], naive_rmw_count, " +
        sharedKvimrEnv.getNaiveRmwCount());
    System.out.println("[KVIMR], naive_read_bytes, " +
        sharedKvimrEnv.getNaiveReadBytes());
    System.out.println("[KVIMR], naive_write_bytes, " +
        sharedKvimrEnv.getNaiveWriteBytes());
    System.out.println("[KVIMR], merged_rmw_count, " +
        sharedKvimrEnv.getMergedRmwCount());
    System.out.println("[KVIMR], merged_read_bytes, " +
        sharedKvimrEnv.getMergedReadBytes());
    System.out.println("[KVIMR], merged_write_bytes, " +
        sharedKvimrEnv.getMergedWriteBytes());
    System.out.println("[KVIMR], merged_logical_update_count, " +
        sharedKvimrEnv.getMergedLogicalUpdateCount());
    System.out.println("[KVIMR], merged_updates_saved, " +
        sharedKvimrEnv.getMergedUpdatesSaved());
    System.out.println("[KVIMR], direct_write_bytes, " +
        sharedKvimrEnv.getDirectWriteBytes());
    System.out.println("[KVIMR], merged_buffer_create_count, " +
        sharedKvimrEnv.getMergedBufferCreateCount());
    System.out.println("[KVIMR], merged_buffer_hit_count, " +
        sharedKvimrEnv.getMergedBufferHitCount());
    System.out.println("[KVIMR], merged_buffer_flush_count, " +
        sharedKvimrEnv.getMergedBufferFlushCount());
  }
